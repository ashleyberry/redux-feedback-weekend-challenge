...
const commentReducer = ( state=[], action ) => { // take comments object from dispatch and updates the state
  if ( action.type === 'SET_COMMENTS' ) { 
    // returning our new redux state "aka store"
    let newComments = action.payload // action.payload === response.data which came from server 
    return newComments
}  
  return state;
}
